"""digest cnn model and create RPC server
install watchdog: monitor the model changes
if model changes, automatically load new model without restart service
"""
...
import news_cnn_model  #pylint: disable=import-error, wrong-import-position
...
MODEL_UPDATE_LAG_IN_SECONDS = 60 * 15 # 15 min
N_CLASSES = 8
n_words = 0
...
class ReloadModelHandler(FileSystemEventHandler):
  def on_any_event(self, event):
    ...
    time.sleep(MODEL_UPDATE_LAG_IN_SECONDS) # wait until all the model files updated
    restoreVars()
    loadModel()
...
def classify(text):
  ...
  # use predict to give news class
  y_predicted = [
